...
boardFromWord :: String -> Board
boardFromWord xs = transpose [[Just x | x <- xs], []]
...
numOcc :: Char -> String -> Int
numOcc c [] = 0
numOcc c (x:xs) = if (x == c) then 1 + numOcc c xs else numOcc c xs
...
submultiset (x:xs) ys = if (elemIndex x ys /= Nothing) then submultiset xs (delete x ys) else Nothing
...
formable :: String -> Rack -> Char -> Maybe String
formable xs ys c = submultiset xs (ys ++ [c])
...
invertBoard :: Board -> Board
invertBoard ys = reverse [ reverse y | y <- ys ]
...
-- Find the number of Nothing to add for each side
addUp (board:xss) acc = if (length [x | x <- [head xs | xs <- (board:xss)], x /= Nothing] /= 0) then (7 - acc) 
                     else addUp zss (acc + 1)
                     where zss = [tail xs | xs <- (board:xss)]
                     
addDown (board:xss) acc = if (length [x | x <- [head (reverse xs) | xs <- (board:xss)], x /= Nothing] /= 0) then (7 - acc) 
                     else addDown zss (acc + 1)
                     where zss = [reverse (tail (reverse xs)) | xs <- (board:xss)]
                     
addLeft (board:xss) acc = if (length [x | x <- board, x /= Nothing] /= 0) then (7 - acc) else addLeft xss (acc + 1)

addRight xss acc = if (length [x | x <- board, x /= Nothing] /= 0) then (7 - acc) else addRight zss (acc + 1)
                   where
                   zss =  reverse (tail (reverse xss))
                   board = head (reverse xss)

-- Group the Nothings from Up, Down, Left And Right                                  
addNothingsUpDown board = (addUp board 0, addDown board 0)
addNothingsLeftRight board = (addLeft board 0, addRight board 0)

-- Add the Nothings for Up and Down
performAddUpDown [] numUp numDown = []
performAddUpDown (b:board) numUp numDown = [ [ Nothing |_ <- [1..numUp]] ++ b ++ [Nothing | _ <- [1..numDown]] ] ++ performAddUpDown board numUp numDown

-- Add the Nothings for Left and Right
performAddLeftRight (board) numLeft numRight =  [ [Nothing | _ <- [1..(length (head board))]] | _ <- [1..numLeft] ] ++ board ++ [ [Nothing | _ <- [1..(length (head board))]] | _ <- [1..numRight] ]

-- Group all functions together
autoResize :: Board -> Board
autoResize board = performAddLeftRight (performAddUpDown board (fst addUpDown) (snd addUpDown)) (fst addLeftRight) (snd addLeftRight)
                  where 
                  addUpDown = addNothingsUpDown board
                  addLeftRight = addNothingsLeftRight board  
...
testBoard = [[Nothing, Just 'a', Nothing], [Just 'b', Nothing, Nothing], [Nothing, Nothing, Just 'c']]

writeMove' :: Move -> Board -> Board
writeMove' = Bram.writeMove . read . show

putLetter :: Board -> (Int, Int) -> Maybe Char -> Board
putLetter board (0, 0) l = [( [l] ++ ( tail (board !! 0) ) )] ++ (tail board)
putLetter board (0, y) l = [( ( take y (board !! 0) ) ++ [l] ++  ( drop ( y+1 ) (board !! 0) ) )] ++ (tail board)
putLetter board (x, 0) l = ( take x board ) ++ [( [l] ++ ( tail (board !! x) ) )] ++ ( drop ( x+1 ) board )
putLetter board (x, y) l = ( take x board ) ++ [( (take y (board !! x)) ++ [l] ++ ( drop (y+1) (board !! x) ) )] ++ ( drop ( x+1 ) board)

writeMovePre :: Move -> [Pos] -> Board -> Board
writeMovePre move [] board = board
writeMovePre move (pos:poss) board = writeMovePre move poss (putLetter board pos (moveLetter move pos))

writeMove :: Move -> Board -> Board
writeMove move board =  autoResize (writeMovePre move (movePositions move) board)
...
replenishRack :: Rack -> LRand Rack
replenishRack rack = replenishRack2 rack (7 - length rack)

replenishRack2 :: Rack -> Int -> LRand Rack
replenishRack2 rack 0 = return rack
replenishRack2 rack n = do
        x <- newLetter
        replenishRack2 (rack ++ [x]) (n-1)
...
allWords1 :: Dict -> Char -> [String]
allWords1 [] c = []
allWords1 (word:dict) c = if (elemIndex c word /= Nothing) then [word] ++ allWords1 dict c else allWords1 dict c
...
allWords2 dict letter l r =  concat [getAllPossibilities word letter l r | word <- (allWords1 dict letter)]

getAllPossibilities word letter l r = getAllPossibilities' word letter l r (getIndecies word letter 0) []

getAllPossibilities' word letter l r [] acc = acc
getAllPossibilities' word letter l r (index:indecies) acc = getAllPossibilities' word letter l r indecies (acc ++ (getPossibility word index l r))

getPossibility :: String -> Int -> Int -> Int -> [(String, Int)]
getPossibility word index leftBorder rightBorder = if ( (index <= leftBorder) && ((length word - 1 - index) <= rightBorder) ) then [(word, index)] else [] 

getIndecies :: String -> Char -> Int -> [Int]
getIndecies [] letter n = []
getIndecies (c:word) letter n = if (c == letter) then [n] ++ getIndecies word letter (n+1) else getIndecies word letter (n+1) 
...
allWords3 :: Dict -> Rack -> Char -> Int -> Int -> [(String, Int)]
allWords3 dict rack letter l r = concat [getAllPossibilities word letter l r | word <- (fromRack (allWords1 dict letter) rack letter)]

fromRack [] rack letter = []
fromRack (xs:xss) rack letter = if (formable xs rack letter == Nothing) then (fromRack xss rack letter) else [xs] ++ (fromRack xss rack letter)
...
letterStream :: LRand LetterStream
letterStream = return [evalRand newLetter (mkSeed i) | i <- [0..]]
...
greedyBestMove :: Dict -> Rack -> Board -> FullMove
greedyBestMove dict rack b = getMaximum [(moveScore move, move) | move <- allMoves dict rack b] 0 (Nothing)

getMaximum [] max acc = acc
getMaximum (xs:xss) max acc = if (fst xs >= max) then getMaximum xss (fst xs) (snd xs) else getMaximum xss max acc
...
--
-- runState (aiOneMove sowpods) (autoResize (boardFromWord "test"),"", "abcdefghijklmnop", (repeat undefined))
--   = (Just ("feedbag",((8,5),V)),(*** Exception: Prelude.undefined
--
-- runState (aiOneMove sowpods) (autoResize (boardFromWord "test"),"", "qqqqqqqqqq", (repeat undefined))
...
aiOneMove dict = do
    ...
    -- Add from letterStream to rack
    let addedToRack = r ++ (take (7 - length r) ls)
    let ls2 = drop (7 - length r) ls
    
    -- Get the best move
    let fullMove = greedyBestMove dict addedToRack b
    
    -- Remove used letters from the rack
    let getWord (Just move@(word, _)) = word
    let getIntersection (Just move) = head [(b !! x !! y) | (x, y) <- (movePositions move), (b !! x !! y) /= Nothing]
    let removeJust (Just c) = c
    let removeIntersection move = if (move /= Nothing) then delete (removeJust (getIntersection move)) (getWord fullMove) else []
    let r' = if (fullMove == Nothing) then take 7 ls2 else addedToRack \\ (removeIntersection fullMove)
    
    -- Play your best move
    let b2 = if (fullMove /= Nothing) then autoResize (writeMove (removeJust fullMove) b) else b
    
    -- Play opponentsMove
    let b' = if ((head oppMovesb) /= Nothing) then autoResize (writeMove (removeJust (head oppMovesb)) b2) else b2
    
    -- Remove first move
    let oppMoves'b = drop 1 oppMovesb
    let oppMoves' = map (read.show) oppMoves'b
    
    --Remove letterStream first seven
    let ls' = if (fullMove == Nothing) then drop 7 ls2 else drop (7 - length r) ls
...
--
--   Bram.connectAI (boardFromWord "test") (convertAItoBram (ai sowpods),"abcdefgqqqqqqqhijklmnopqrstuvwxyz") (Bram.ai sowpods,"abcdefghijklmnopqrstuvwxyz")
...
aiHead :: Dict -> Board -> LetterStream -> [FullMove] -> [FullMove]
aiHead dict board ls oppMoves = [fullMove] ++ (aiTail dict b' r' ls' oppMoves')
                                where
                                (fullMove, (b', r', ls', oppMoves')) = runState (aiOneMove dict) (autoResize board, "", ls, oppMoves)
                      
aiTail :: Dict -> Board -> Rack -> LetterStream -> [FullMove] -> [FullMove]       
aiTail dict board r ls oppMoves = [fullMove] ++ (aiTail dict b' r' ls' oppMoves')
                                  where
                                  (fullMove, (b', r', ls', oppMoves')) = runState (aiOneMove dict) (autoResize board, r, ls, oppMoves)
...
--
-- connectAI b (ai sowpods, "abcdefgqqqqqqqhijklmnopqrstuvwxyzabcdefghijkl" ++ undefined) (ai sowpods, "abcdefghijklmnopqrstuvwxyzabcdefghijkl" ++ undefined)
--
...
--
-- connectAI (autoResize $ boardFromWord "test") (ai sowpods, "qqqqqqqabcdefghijklmn" ++ undefined) (ai sowpods, "qqqqqqqabcdefghijklmn"++ undefined)
--
...
connectAI :: Board -> (AI, LetterStream) -> (AI, LetterStream) -> [FullMove]
connectAI b (ai1, ls1) (ai2, ls2) = connectAI2 b (ai1,ls1,[]) (ai2,ls2,[]) 1

connectAI2 b (ai1,ls1,fm1) (ai2,ls2,fm2) n = ((last m1):[(last m2)]) ++ (connectAI2 b (ai1,ls1,m1) (ai2,ls2,m2) (n+1))
                                      where 
                                      m1 = take n (ai1 b ls1 (fm2 ++ [Nothing]))
                                      b' = if move == Nothing then b else writeMove (removeJust move) b where move = head m1
                                      m2 = take n (ai2 b' ls2 ((tail m1) ++ [Nothing]))
-- Utility code given to you:
--
